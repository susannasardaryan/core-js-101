...
function doRectanglesOverlap(rect1, rect2) {
  ...
  let width = ((rect2.left <= right1) && (right1 <= right2));
  width = width || ((rect1.left <= right2) && (right2 <= right1));
  let height = ((rect2.top <= bottom1) && (bottom1 <= bottom2));
  height = height || ((rect1.top <= bottom2) && (bottom2 <= bottom1));
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${first}${a > b ? b : a}, ${a > b ? a : b}${end}`;
}
...
function getMatrixProduct(m1, m2) {
  ...
  if (colsM1 !== rowsM2) throw new Error('Incompatible matrix sizes for multiplication');
